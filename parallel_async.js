ES7 brings async/await to, for the most part, replace promise chains for asynchronous function calls. But promises still have some fancy tricks up their sleeves that can't be abstracted out.<br />
<br />
<b>
...
<br />
<pre data-codetype="auto"><code>const sendText = text =&gt; {
...
<br />
<pre data-codetype="auto"><code>const text = await sendText('Sally')
...
<br />
<pre data-codetype="auto"><code>const text = await sendText('Sally')
const otherText = await sendText(' loves dogs') //doesn't rely on the function above it
...
<br />
<pre data-codetype="auto"><code>const text = sendText('Sally')
const otherText = sendText(' loves dogs')
const [first, second] = await Promise.all([text, otherText])
//checkpoint, using Promise.all, to ensure all promises are resolved before continuing
